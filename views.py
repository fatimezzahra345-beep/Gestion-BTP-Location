                ...
                with b2:
                    ...
                    new_stat = st.selectbox("Statut",
                                             ...
                                             index=["disponible","loue","maintenance","commande"].index(
                                                 ...
                                                 ["disponible","loue","maintenance","commande"] else 0)
...
def render_clients():
    ...
    tab_liste, tab_ajouter = st.tabs(["Clients","Nouveau Client"])
...
def render_devis():
    ...
    with tab_liste:
        ...
        if not devis_list:
            ...
            for col,(lbl,f,c) in zip([k1,k2,k3,k4],[
                ...
                with col:
                    val = f"{sum(d['montant_ttc'] for d in devis_list):,.0f}" if f is None else str(sum(1 for d in devis_list if f(d)))
                ...
                if ddata:
                    ...
                    with ca:
                        if ddata["statut"]=="brouillon" and st.button("Valider",type="primary",
                            ...
                            ctrl.valider_devis(d_ids[sel]); st.success("Devis validé !"); st.rerun()
                    with cb:
                        if ddata["statut"]=="valide" and st.button("Creer Facture",
                                                                     width='stretch',key="dv_fac"):
                            _,fn=ctrl.create_facture(d_ids[sel]); st.success(f"Facture {fn} créée !"); st.rerun()
                    with cc:
                        if ddata["statut"] not in ["annule","facture"] and st.button("Annuler",
                                                                                      width='stretch',key="dv_ann"):
                            ctrl.annuler_devis(d_ids[sel]); st.rerun()
                    with cd:
                        try:
    ...
    with tab_nouveau:
        ...
        with st.form("form_devis_new"):
            ...
            c1,c2,c3 = st.columns([2,1,1])
            with c1: client_s = st.selectbox(" Client *",list(client_opts.keys()))
            with c2: dd = st.date_input("📅 Date début *",value=date.today())
            with c3: df = st.date_input("📅 Date fin *",value=date.today()+timedelta(days=1))
    ...
    with tab_modifier:
        ...
        if dd_data:
            ...
            with st.form(f"form_mod_{b_map[sel_mod]}"):
                c1,c2,c3 = st.columns([2,1,1])
                with c1: cm = st.selectbox(" Client",list(c_opts.keys()),index=cidx)
                with c2: dm = st.date_input("📅 Début",value=dd_data["date_debut"])
                with c3: fm = st.date_input("📅 Fin",value=dd_data["date_fin"])
                ct,_ = st.columns([1,2])
                with ct: tva_m2 = st.number_input("TVA (%)",0.0,100.0,float(dd_data["tva_taux"]),1.0)
                _sec("Engins (pré-sélectionnés selon le devis actuel)")
                actuels={l["description"]:l["quantite"] for l in dd_data.get("lignes",[])}
                lgn_m=[]
                for eng in engins:
                    in_d=eng.nom in actuels; qd=actuels.get(eng.nom,1.0)
                    mm1,mm2 = st.columns([3,1])
                    with mm1: chk=st.checkbox(f"{eng.nom} — {eng.prix_journalier:,.0f} MAD/jr",
                                              value=in_d,key=f"md_s_{eng.id}")
                    with mm2: qty=st.number_input("Jours",1.0,999.0,float(qd),1.0,
                ...
                if lgn_m:
                    ...
                    r1,r2,r3=st.columns(3)
                    r1.metric("HT",format_mad(ht2)); r2.metric("TVA",format_mad(tv2))
                    r3.metric("TTC",format_mad(tt2))
                save_m=st.form_submit_button("Enregistrer les modifications",type="primary",
                                              width='stretch')
            if save_m:
                if not lgn_m: st.error("⚠️ Au moins un engin requis.")
                elif fm<=dm:  st.error("⚠️ Date fin doit être après date début.")
                else:
                    try:
...
def render_livraison():
    ...
    with tab_liste:
        ...
        if not bls:
            ...
            with c1: sel_bl=st.selectbox("Télécharger un BL PDF",["—"]+list(bl_map.keys()))
            with c2:
                if sel_bl!="—":
                    ...
                    if bl_d:
                        try:
                            ...
                            st.markdown("<br>",unsafe_allow_html=True)
                            st.download_button("Telecharger PDF",data=pdf,
                                file_name=f"BL_{sel_bl}.pdf",mime="application/pdf",key="dl_bl")
                        except Exception as e: st.error(str(e))
...
def render_attachements():
    ...
    with tab_liste:
        ...
        if not atts:
            ...
            c1,c2=st.columns([3,1])
            with c1: sel_att=st.selectbox("Télécharger PDF",["—"]+list(att_map.keys()))
            with c2:
                if sel_att!="—":
    ...
    with tab_creer:
        ...
        with ca1: sel_d2=st.selectbox(" Devis *",list(d_opts.keys()))
        with ca2: mois_a=st.number_input("Mois",1,12,date.today().month)
        with ca3: annee_a=st.number_input("Année",2020,2035,date.today().year)
        cb1,cb2=st.columns(2)
        with cb1: sel_eng=st.selectbox(" Engin *",list(e_opts.keys()))
        with cb2: projet_a=st.text_input("🏗️ Projet *",placeholder="ex: Place Jamaa El Fna")
        ...
        with st.form("form_att_new"):
            ...
            hdr[0].markdown("**Jour**"); hdr[1].markdown("**Heures**"); hdr[2].markdown("**Jour (0/1)**")
...
def render_facturation():
    ...
    with tab_liste:
        ...
        if not factures:
            ...
            c1,c2=st.columns([3,1])
            with c1: sel_pdf=st.selectbox(" Télécharger PDF",["—"]+list(all_map.keys()))
            with c2:
                if sel_pdf!="—":
                    ...
                    if fd:
                        try:
                            ...
                            st.markdown("<br>",unsafe_allow_html=True)
                            st.download_button("Telecharger PDF",data=pdf,
                                file_name=f"Fac_{sel_pdf}.pdf",mime="application/pdf",key="fac_dl")
                        except Exception as e: st.error(str(e))
    ...
    with tab_devis:
        ...
        if not dvs:
            ...
            with st.form("form_fac_d"):
                ...
                c1,c2=st.columns(2)
                with c1: ech_d=st.date_input("📅 Échéance *",value=date.today()+timedelta(days=30))
                with c2: tva_d=st.number_input("TVA (%)",0.0,100.0,20.0,1.0)
                if st.form_submit_button("Creer la Facture",type="primary",width='stretch'):
                    try:
                        ...
                        s=SessionLocal(); fo=s.query(FM).filter(FM.id==fi).first()
                        fo.echeance=ech_d; fo.tva_taux=tva_d; s.commit(); s.close()
    ...
    with tab_att:
        ...
        if not atts2:
            ...
            with st.form("form_fac_att2"):
                ...
                with c1: ech_a=st.date_input("📅 Échéance *",value=date.today()+timedelta(days=30))
                with c2: tva_a=st.number_input("TVA (%)",0.0,100.0,20.0,1.0)
                with c3: red_a=st.number_input("Réduction (%)",0.0,100.0,0.0,1.0)
    ...
    with tab_mod:
        ...
        if not fmod:
            ...
            with st.form(f"form_fm_{fm_map[sel_m]}"):
                ...
                lgn_fm=[]
                for eng in engs2:
                    in_f=eng.nom in la; qd=la.get(eng.nom,1.0); pd_=pu.get(eng.nom,eng.prix_journalier)
                    m1,m2,m3=st.columns([3,1,1.5])
                    with m1: chk=st.checkbox(eng.nom,value=in_f,key=f"fm3_c_{eng.id}")
                    with m2: qty=st.number_input("Jours",1.0,999.0,float(qd),1.0,
                                                  key=f"fm3_q_{eng.id}",label_visibility="collapsed")
                    with m3: pu2=st.number_input("Prix",0.0,99999.0,float(pd_),50.0,
                ...
                if st.form_submit_button("Enregistrer",type="primary",width='stretch'):
                    if not lgn_fm: st.error("⚠️ Au moins un engin requis.")
                    else:
                        ctrl.update_facture_lignes(fm_map[sel_m],lgn_fm,tva_fm)
                        st.success("✅ Facture mise à jour !"); st.rerun()
    ...
    with tab_red:
        ...
        if not fr:
            ...
            if st.button("Appliquer la reduction",type="primary",key="btn_red3"):
                ...
                st.success("✅ Réduction appliquée !"); st.rerun()
...
def render_paiements():
    ...
    if retards:
        _sec("Factures en Retard — Action Requise")
        for p in retards:
            st.markdown(f"""
    ...
    if not non_p:
        ...
        with c1: sel_p=st.selectbox("Facture",list(fo.keys()),key="pay_sel2")
        ...
        with c3:
            ...
            if st.button("Valider",type="primary",width='stretch',key="pay_btn2"):
                ...
                st.success(f"✅ {mnt:,.2f} MAD enregistré !"); st.rerun()
...
def render_attestations():
    ...
    with tab_liste:
        ...
        if not atts:
            ...
            c1,c2=st.columns([3,1])
            with c1: sel_a=st.selectbox("Télécharger PDF",["—"]+list(am.keys()))
            with c2:
                if sel_a!="—":
...
def render_admin():
    ...
    tab_mdp,tab_info=st.tabs(["Changer le Mot de Passe","Systeme"])
...
def render_engins_dispo_widget():
    ...
    for i,e in enumerate(engins_d):
        total=e["total"]; dispo=e["disponibles"]; loues=e["loues"]; cmds=e["commandes_count"]
